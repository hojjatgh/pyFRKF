...
obs0 = observation_model(simul_obs, H,lin)
obs0 = obs0.reshape(len(obs0),1)
n_step = 1
### linear observation
obs = obs0 + np.random.normal(0,np.sqrt(theta_r), (len(obs0),n_step))
#### Nonlinear Observation
#obs = np.sqrt(obs0) + np.random.normal(0,np.sqrt(theta_r), (len(obs0),n_step))
...
rank = 12
prob = CSKF(forward_model, observation_model, initial_dist, params, H, R, Q, P, nx, n_step, rank, ref_bathy, obs,lin)
...
savemat('results_lin96.mat',{'results':results, 'uncert': uncert})
